...
| 文件 | 用途 |
|---|---|
| `lib/flight/flight-data.json` | 完整航班计划历史快照（班次、时刻、班期规则） |
| `lib/flight/airport-coordinates.ts` | 机场经纬度（基础地理数据） |
| `lib/flight/airport-city-map.ts` | 机场 ↔ 城市映射 |
| `public/airport.csv` | 机场三字码基础数据 |

## 航班数据字段说明 / Flight Data Schema

`lib/flight/flight-data.json` 中每条航班记录的主要字段（对应 `lib/flight/types.ts` 中的 `Flight` 接口）：
...
| **航班数据（`lib/flight/flight-data.json`）** | 由作者整理自海航官方公开发布的航班计划。**作者对该数据集本身不附加任何额外的版权主张**，也**不对其准确性、完整性、时效性做任何保证**。该数据为历史快照，仅供学习/参考 |
| **机场基础数据（`public/airport.csv`、`lib/flight/airport-coordinates.ts`）** | IATA 三字码、机场中文名、经纬度等为公开通用的民航地理信息，本项目仅作引用整理 |
...
| **Flight data (`lib/flight/flight-data.json`)** | A manually-compiled historical snapshot from publicly available Hainan Airlines schedules. The author asserts no additional copyright claim on the dataset itself, and likewise makes NO warranty as to its accuracy, completeness, or timeliness. The data is a frozen snapshot, intended for learning and reference only |
